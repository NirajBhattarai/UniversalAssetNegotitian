...
export default function MultiAgentChat() {
  ...
  const [chatMode, setChatMode] = useState<
    'agents' | 'hedera' | 'negotiation' | 'wallet-balance'
  >('agents');
  ...
  const [hederaConnectionStatus, setHederaConnectionStatus] = useState<
  ...
  const [walletAgentStatus, setWalletAgentStatus] = useState<
  ...
  const initializeHederaClient = async () => {
    try {
      ...
      if (data.connected) {
        ...
        addHederaMessage(
          'assistant',
          '🔗 Connected to Hedera network! I can help you check your HBAR balance, query account information, and perform basic Hedera operations. How can I assist you?'
        );
      } else {
        ...
        addHederaMessage(
          'assistant',
          '❌ Failed to connect to Hedera network. Please check your configuration and try again.'
        );
      }
    } catch (error) {
      ...
      addHederaMessage(
        'assistant',
        '❌ Error initializing Hedera client. Please check your environment variables.'
      );
    }
  };
  ...
  const addNegotiationStep = (
    agent: string,
    message: string,
    type: NegotiationStep['type']
  ) => {
  ...
  const addThinkingStep = (agent: string, duration: number) => {
    ...
    setTimeout(() => {
      setNegotiationSteps(prev =>
        prev.filter(step => step.id !== thinkingStep.id)
      );
    }, duration);
  };
  ...
  const simulateNegotiation = async (userRequest: string) => {
    ...
    try {
      ...
      addNegotiationStep(
        ...
        '🔍 Received your carbon credit request. Analyzing marketplace...',
        'status'
      );
      ...
      if (negotiationResponse.success && negotiationResponse.response) {
        ...
        addNegotiationStep(
          'Carbon Credit Negotiation Agent',
          `❌ Error: ${negotiationResponse.error || 'No response from Negotiation Agent'}`,
          'response'
        );
      }
      ...
      addNegotiationStep(
        ...
        '🔄 Processing payment for carbon credits...',
        'status'
      );
      ...
      if (paymentResponse.success && paymentResponse.response) {
        ...
        addNegotiationStep(
          'Carbon Credit Payment Agent',
          `❌ Error: ${paymentResponse.error || 'No response from Payment Agent'}`,
          'response'
        );
      }
      ...
      addNegotiationStep(
        'System',
        '🤝 Carbon credit transaction completed!\n\nAll agents have been contacted and responses received. Check individual agent responses above for detailed information.',
        'result'
      );
    } catch (error) {
      addNegotiationStep(
        'System',
        `❌ Error during negotiation: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'result'
      );
    } finally {
  ...
  const sendMessageToAgent = async (message: string, agentId: string) => {
    ...
    try {
      ...
      addMessage(
        'agent',
        `❌ Error communicating with ${agent.name}: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'System'
      );
    }
  };
  ...
  const handleWalletBalanceCheck = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addMessage(
          'agent',
          `💰 Checked balance for ${walletAddress} - Total: $${data.totalUsdValue.toFixed(2)}`,
    ...
    <div className='bg-white rounded-2xl shadow-xl border border-gray-200 h-full flex overflow-hidden'>
      ...
      <div className='w-64 border-r border-gray-200 bg-gradient-to-b from-gray-50 to-white flex flex-col'>
        ...
        <div className='flex-1 overflow-y-auto p-3 space-y-2'>
          ...
          {agents.map(agent => (
            ...
            >
              ...
              <div className='flex-1 text-left'>
                ...
                <div
                  className={`text-xs ${selectedAgent === agent.id && chatMode === 'agents' ? 'text-white/80' : 'text-gray-500'}`}
                >
                  {agent.description}
                </div>
              </div>
          ...
          <div className='text-xs font-semibold text-gray-500 uppercase px-3 py-2 mt-4'>
            Special Modes
          </div>
          <button
          ...
          >
            ...
            <div className='flex-1 text-left'>
              ...
              <div
                className={`text-xs ${chatMode === 'negotiation' ? 'text-white/80' : 'text-gray-500'}`}
              >
          ...
          >
            ...
            <div className='flex-1 text-left'>
              ...
              <div
                className={`text-xs ${chatMode === 'hedera' ? 'text-white/80' : 'text-gray-500'}`}
              >
          ...
          >
            ...
            <div className='flex-1 text-left'>
              ...
              <div
                className={`text-xs ${chatMode === 'wallet-balance' ? 'text-white/80' : 'text-gray-500'}`}
              >
                Multi-network balances
              </div>
            </div>
            {walletAgentStatus === 'online' && (
              <CheckCircle className='w-4 h-4 text-green-500' />
            )}
            {walletAgentStatus === 'offline' && (
              <AlertCircle className='w-4 h-4 text-red-500' />
            )}
            {walletAgentStatus === 'checking' && (
              ...
              {negotiationSteps.length === 0 ? (
                <div className='text-center text-gray-500 py-12'>
                  ...
                  <p className='text-xl font-semibold mb-3'>
                    Live Multi-Agent Carbon Credit Negotiation
                  </p>
                  <p className='text-sm'>
                    Watch agents negotiate carbon credit deals in real-time!
                    Try: "Find 10,000 carbon credits at best price"
                  </p>
                </div>
              ) : (
                negotiationSteps.map(step => (
                  <div key={step.id} className='animate-fadeIn'>
                    <div
                      className={`flex ${step.agent === 'User' ? 'justify-end' : 'justify-start'}`}
                    >
                      <div
                        className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl shadow-sm ${
                          step.agent === 'User'
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value='polygon'>
                          Polygon Network (MATIC, USDC, USDT)
                        </option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div className='bg-white rounded-xl border border-gray-200 p-6 shadow-sm'>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-6'>
                      ...
                      <div>
                        ...
                        <span className='ml-2 text-gray-900'>
                          {Object.keys(balanceResult.networks).length}
                        </span>
                      </div>
                    </div>

                    {/* Network Results */}
                    {Object.entries(balanceResult.networks).map(
                      ([networkId, networkData]) => (
                        ...
                        >
                            ...
                            <div className='space-y-2'>
                              <div className='flex items-center gap-2'>
                                ...
                                <span className='font-medium'>
                                  {networkData.nativeBalance.token.name}
                                </span>
                                <span
                                ...
                                <div>
                                  ...
                                  <span className='ml-2 font-mono'>
                                    {formatBalance(
                                      networkData.nativeBalance
                                        .balanceFormatted,
                                      networkData.nativeBalance.token.decimals
                                    )}{' '}
                                    {networkData.nativeBalance.token.symbol}
                                  </span>
                                </div>
                                <div>
                                  <span className='text-gray-600'>
                                  ...
                                  <span className='ml-2 font-semibold'>
                                    $
                                    {networkData.nativeBalance.usdValue.toFixed(
                                      2
                                    )}
                                  </span>
                                </div>
                          ...
                          {networkData.tokenBalances &&
                            networkData.tokenBalances.length > 0 && (
                              <div className='space-y-3'>
                                <h5 className='font-medium text-gray-600'>
                                  ERC20 Tokens:
                                </h5>
                                {networkData.tokenBalances.map(
                                  (tokenBalance, index) => (
                                    <div
                                      key={index}
                    ...
                    <div className='border-t pt-4'>
                      ...
                      <div className='flex gap-2'>
                        {Object.values(balanceResult.networks).some(
                          network => network.nativeBalance?.token.isNative
                        ) && (
                        ...
                        {Object.values(balanceResult.networks).some(
                          network =>
                            ...
                            network.tokenBalances.length > 0
                        ) && (
        ...
        >
          <div className='flex space-x-3'>
            ...
            <button
              ...
              disabled={
                !input.trim() || isLoading || isHederaLoading || isNegotiating
              }
