...
export default function HederaChat({ className = '' }: HederaChatProps) {
  ...
  const initializeServices = async () => {
    try {
      ...
      if (hederaConnected) {
        ...
        let welcomeMessage =
          '🔗 Connected to Hedera network! I can help you check your wallet balances, query account information, and perform basic Hedera operations.';
      ...
      addMessage(
        'assistant',
        '❌ Error initializing services. Please check your environment variables.'
      );
    }
  };
    ...
    try {
      ...
      if (
        (lowerMessage.includes('balance') ||
          ...
          lowerMessage.includes('what tokens')) &&
        walletBalanceService.current
      ) {
  ...
  const handleBalanceCheck = async () => {
    ...
    if (!isWalletConnected || !address) {
      addMessage(
        'assistant',
        '❌ Please connect your wallet first to check balances.'
      );
      return;
    }
    ...
    try {
      ...
      if (response.success && response.balances.length > 0) {
        ...
        addMessage(
          'assistant',
          `❌ Failed to fetch balances: ${response.error || 'Unknown error'}`
        );
      }
    } catch (error) {
        ...
        {messages.length === 0 ? (
          <div className='text-center text-gray-500 py-12'>
            ...
            <p className='text-xl font-semibold mb-3'>
              Welcome to Hedera AI Assistant
            </p>
            <p className='text-sm'>
              Ask me about your HBAR balance, account info, or any Hedera
              operations!
            </p>
          </div>
        ) : (
