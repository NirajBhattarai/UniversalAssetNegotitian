...
export class A2AAgentService {
  ...
  private agents: AgentConfig[] = [
    ...
    },
    {
      ...
      description:
        'Check wallet balances across Hedera, Ethereum, and Polygon networks',
    },
  ];
  ...
  constructor() {
    ...
    this.initializeClients().catch(error => {
      console.warn(
        'Some A2A agents are offline - this is expected if agents are not running:',
        error
      );
    });
  }
    ...
    await Promise.all(
      this.agents.map(async agent => {
        try {
          ...
          console.warn(
            `⚠️  ${agent.name} is offline (this is normal if agents are not running):`,
    ...
    try {
      ...
      if (!client) {
        try {
          ...
          return {
            success: false,
            error: `Failed to connect to ${agent.name}: ${error instanceof Error ? error.message : 'Unknown error'}`,
          };
        }
      }
      ...
      for await (const event of stream) {
        if (event.kind === 'status-update') {
          const statusText =
            (event.status.message?.parts?.[0] as any)?.text || '';
          if (statusText) {
  ...
  async sendMessageToAllAgents(
    ...
    Record<string, { success: boolean; response?: string; error?: string }>
  > {
    ...
    await Promise.all(
      this.agents.map(async agent => {
        results[agent.id] = await this.sendMessageToAgent(
          message,
          agent.id,
          contextId
        );
