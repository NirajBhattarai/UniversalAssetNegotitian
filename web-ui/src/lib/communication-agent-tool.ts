  ...
  return tool(
    async ({ 
      ...
      try {
        ...
        const workflowType = workflow || communicationAgentHelpers.determineWorkflowType(request);
        ...
        switch (workflowType.toLowerCase()) {
          case 'carbon_credit_purchase':
            result += await communicationAgentHelpers.executeCarbonCreditPurchaseWorkflow(request, context);
            break;
            
          case 'portfolio_analysis':
            result += await communicationAgentHelpers.executePortfolioAnalysisWorkflow(request, context);
            break;
            
          case 'transaction_monitoring':
            result += await communicationAgentHelpers.executeTransactionMonitoringWorkflow(request, context);
            break;
            
          case 'agent_health_check':
            result += await communicationAgentHelpers.executeAgentHealthCheckWorkflow();
            break;
            
          case 'custom_workflow':
            result += await communicationAgentHelpers.executeCustomWorkflow(request, agents, context);
            break;
            
          default:
            result += await communicationAgentHelpers.executeGeneralWorkflow(request, context);
        }
...
- "Check status of all agent services"`,
      schema: z.object({
        ...
        workflow: z
          .string()
          .optional()
          .describe('Specific workflow type: carbon_credit_purchase, portfolio_analysis, transaction_monitoring, agent_health_check, custom_workflow')
          .default('general'),
        agents: z
          .string()
          .optional()
          .describe('Specific agents to involve (comma-separated): wallet_balance, carbon_credit, payment, all')
          .default('all'),
        context: z
          .string()
          .optional()
...
const communicationAgentHelpers = {
  determineWorkflowType: (request: string): string => {
    ...
    } else if (lowerRequest.includes('portfolio') || lowerRequest.includes('analyze')) {
      return 'portfolio_analysis';
    } else if (lowerRequest.includes('monitor') || lowerRequest.includes('track')) {
      return 'transaction_monitoring';
    } else if (lowerRequest.includes('health') || lowerRequest.includes('status')) {
  ...
  executeCarbonCreditPurchaseWorkflow: async (request: string, context?: string): Promise<string> => {
  ...
  executePortfolioAnalysisWorkflow: async (request: string, context?: string): Promise<string> => {
  ...
  executeTransactionMonitoringWorkflow: async (request: string, context?: string): Promise<string> => {
  ...
  executeCustomWorkflow: async (request: string, agents?: string, context?: string): Promise<string> => {
  ...
  executeGeneralWorkflow: async (request: string, context?: string): Promise<string> => {
